...
In the previous chapter, [Display](3.md), we learned how to control the micro:bit's LED matrix to display patterns and messages. But what if we want to *interact* with the outside world? What if we want to connect a button, a sensor, or a motor to our micro:bit? That's where GPIO pins come in!
...
*  `use embedded_hal::digital::v2::InputPin;`: We need to import the `InputPin` trait from the `embedded-hal` crate to work with input pins. This is an interface defined in [HAL (Hardware Abstraction Layer)](5.md).
...
3.  **Hardware Register:** The `GPIO Pin` communicates with the hardware register in the microcontroller to configure the pin as an input with a pull-up resistor or to read the current value of the pin. The `into_pull_up_input()` and `is_low()` will be abstracted by [HAL (Hardware Abstraction Layer)](5.md).
...
In the next chapter, we'll explore [HAL (Hardware Abstraction Layer)](5.md), which provides a unified interface to access all hardware peripherals such as `GPIO`, `Display` and `Timer`.
